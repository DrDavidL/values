...
def check_password(widget_key_suffix="") -> bool:
    ...
    if not st.session_state.password_correct:
        ...
        if st.session_state.login_attempts > 0:
            st.error(f"Incorrect password. Attempts: {st.session_state.login_attempts}")
...
with st.expander("About this App"):
    ...
    st.info(
        "If you have an optional password, AI versions of your value statements are re-phrased using the [Gemini Language Model](https://aistudio.google.com/): "
    )
...
categories = {
    ...
    "X. Aesthetics & Appreciation": ["Beauty", "Appreciation", "Grace"],
...
with col3:
    ...
    if final_selection:
        ...
        if len(selected_cores) == 2:
            st.write(
                "## Step 3: Select Tier 2 Values to complement your primary Tier 1 Values"
            )
            st.write(
                "Where it makes sense to you, add Tier 2 values to complement your Tier 1 Values."
            )

            # Filter out Tier 1 values from the Tier 2 selection options
            tier2_options = [
                value for value in unique_values if value not in selected_cores
            ]
            ...
            statement1 = f"I value {selected_cores[0]} supported by " + ", ".join(
                bucket1
            )
            statement2 = f"I value {selected_cores[1]} supported by " + ", ".join(
                bucket2
            )
            ...
            ai_version = st.checkbox(
                ...
                value=False,
                help="If you have a password, you can use AI to enhance your draft statements.",
            )
            if ai_version:
    ...
    if (
        "draft_statements" in st.session_state
        or "ai_draft_statements" in st.session_state
    ):
        ...
        def generate_docx():
            ...
            draft = st.session_state.get(
                "draft_statements", "No draft statements generated."
            )
            document.add_paragraph(draft)

            ai = st.session_state.get(
                "ai_draft_statements", "No AI draft statements generated."
            )
